...
The rule I set early: match the photographs, not "pretty shader" defaults. Every WebGL demo on the internet converges on the same teal water and purple sky. The actual Adriatic on a July morning is different. The water is a deep azure with a calm swell, the headlands are muted green under haze, and the far ranges are grey crags, not gradients.
...
There are two scenes, really. The default is night: a deep indigo duotone with a gibbous moon and warm golden light necklaces along both shores, because the real town at night is sodium lamps, not grey noir. Day is that backlit sunrise.
...
Then I started listing who else browses without a GPU. Virtual machines. Remote desktop sessions. Corporate laptops with acceleration disabled by policy or by a broken driver. For all of them, WebGL "works" exactly the way it works for Lighthouse: silently, on the CPU, one slow frame at a time.
